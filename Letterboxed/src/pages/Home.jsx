    ...
    <div className={`${isAuth ? 'bg-gradient-to-b from-[#1E252C]/50 via[#1E252C] via-5% to-[#14181C] pt-[30px]' : 'bg-[#14181C]'}`}>
      ...
      <div className='w-full flex justify-center'>
        <div className='xl:w-[950px] '>
          ...
          <h1 className={`${!isAuth && 'hidden'} w-full pb-[5px] border-b mb-[10px] border-[#456] text-[#9AB] text-[12px] tracking-[0.075em] graReg ${!isAuth && "hidden"}`}>POPULAR ON LETTERBOXD</h1>
          ...
          {!isAuth && <><h1 className='text-[13px] tracking-[0.075em] graReg mt-5 mb-2.5 text-[#99AABB]'>LETTERBOXD LETS YOU...</h1>
            <Guide /></>}

          <h1 className='w-full pb-[5px] border-b mb-[10px] border-[#456] text-[#9AB] text-[12px] tracking-[0.075em] graReg'>AND THE OSCAR GOES TO...</h1>
          <div className='grid grid-cols-10 gap-1 my-[3px]'>
            {oscars?.data?.map((item) => {
              return <PosterCard key={item._id} item={item} />
            })}
          </div>
          <p className='mb-2.5 mt-2 text-[#99AABB] text-[15px] pb-[64px] graReg'>The nominees for Best Motion Picture of the Year at the 97th Academy Awards, hosted on March 2, 2025. Follow the <Link className='text-[#DDEEFF]' to={"https://www.oscars.org/"}>Oscars</Link>.</p>


          {isAuth && <>
            <h1 className='w-full pb-[5px] border-b mb-[20px] border-[#456] text-[#9AB] text-[12px] tracking-[0.075em] graReg '>LATTEST NEWS</h1>
            <div className='w-full h-[175px] mb-[96px] flex '>
              ...
              <div className='bg-[#202830] h-full w-full rounded-r-md flex justify-center items-center'>
                <div className='w-[580px]'>
                  <h1 className='graReg text-[22px] font-bold tracking-wide mb-[5px]'>{news?.news?.[0]?.title}</h1>
          ...
          <div className={`${!isAuth && 'flex justify-between'} `}>
            <div className={`${!isAuth && 'w-[630px]'}`}>
              <h1 className='w-full pb-[5px] border-b mb-[20px] border-[#456] text-[#9AB] text-[12px] tracking-[0.075em] graReg '>POPULAR REVIEWS THIS WEEK</h1>
            ...
            <div className={`${!isAuth ? 'w-[230px]' : 'w-full  mt-[64px]'}`}>
              <h1 className='w-full pb-[5px] border-b mb-[20px] border-[#456] text-[#9AB] text-[12px] tracking-[0.075em] graReg '>POPULAR LIST</h1>
              <div className={`w-full ${isAuth && 'flex gap-14'}`}>
                {popLists?.data?.map((item,index) => {
                  return <Link to={`/list/${item?._id}`} key={index}><div className={`${isAuth ? "w-[256px]" : "w-full"}`}><ListCard item={item} /></div></Link>
                })}
              </div>
